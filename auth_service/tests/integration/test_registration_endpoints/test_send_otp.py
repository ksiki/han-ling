import pytest
from httpx import AsyncClient

SEND_OTP_URL = "/api/v1/register/send-otp"


@pytest.mark.asyncio
async def test_send_otp_success(
    client: AsyncClient, fake_redis, mock_captcha_success, mock_email_success
):
    """Успешное начало регистрации с отправкой OTP."""
    response = await client.post(
        SEND_OTP_URL,
        json={
            "email": "new_user@example.com",
            "password": "StrongPass123!",
            "captcha_token": "dummy_valid_token",
        },
    )

    assert response.status_code == 200


@pytest.mark.asyncio
async def test_send_otp_invalid_captcha(
    client: AsyncClient, fake_redis, mock_captcha_fail
):
    """Отказ при невалидной капче."""
    response = await client.post(
        SEND_OTP_URL,
        json={
            "email": "new_user@example.com",
            "password": "StrongPass123!",
            "captcha_token": "invalid_token",
        },
    )

    assert response.status_code == 400


@pytest.mark.asyncio
async def test_send_otp_email_exists(
    client: AsyncClient, test_user, fake_redis, mock_captcha_success
):
    """Отказ при регистрации с уже занятым email."""
    response = await client.post(
        SEND_OTP_URL,
        json={
            "email": test_user.email,
            "password": "StrongPass123!",
            "captcha_token": "dummy_valid_token",
        },
    )

    assert response.status_code == 409
