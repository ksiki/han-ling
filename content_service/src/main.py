...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    """
    logger.info(f"Запуск миксросевиса {config.PRODUCT_NAME} Content...")
    await init_redis()
    yield
    await async_engine.dispose()
    await close_redis()
    logger.info(f"Микросервис {config.PRODUCT_NAME} Content выключен")


app = FastAPI(
    title=f"{config.PRODUCT_NAME} Content",
    description="...",
    version="1.0.0",
    root_path="/content",
